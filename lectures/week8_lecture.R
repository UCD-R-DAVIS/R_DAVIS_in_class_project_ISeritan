...
sample_dates_lub
#went well bc the dates were already in ymd
...
# --> 2016-01-01 12:00:00 PST
#24 hour time:
lubridate::ymd_hm("2016/04/05 14:47", 
                  tz="America/Los_Angeles")
# --> 2016-04-05 14:47:00 PDT - note that it takes the timezone from when the date itself was. also 24hr time can be more useful than 12hr time bc you can see the earlier vs later times more easily
...
months <- month(mloa2$datetime, label = TRUE, abbr=TRUE) #abbr=FALSE gives full month names
table(months)
...
#gives you 2 months prior to the given date

decimal_date(mdy('10-01-2000'))
#can be a helpful way to deal with dates although it seems weird to see dates as decimals
...
f_to_k(72) #this now returns k. useful once we start writing iterative functions and we're creating multiple values, all of which we want to see

# source()ing functions ---------------------------------------------------

#can make a file called functions with all the functions you want to save, then run source("scripts/functions.R") to pull all the functions out of it and into your environment
...
#can generalize this for any country and any year range
